...
public class GameController {
    ...
    public ResponseEntity<Void> revealQuestion(@PathVariable Long contestId, @PathVariable Long contestQuestionId,
                                               @RequestParam(defaultValue = "1") Integer orderIndex,
                                               @RequestParam(defaultValue = "Sample Question?") String question) {
        log.info("Revealing question {} for contest {}", contestQuestionId, contestId);
        eventProducer.publishQuestionRevealed(contestId, contestQuestionId, orderIndex, question);
        return ResponseEntity.ok().build();
    }
    ...
    public ResponseEntity<Void> refreshLeaderboard(@PathVariable Long contestId) {
        ...
        var ev = GameEventDTO.LeaderboardRefreshEvent.builder()
                .eventType("leaderboard.refresh").contestId(contestId).timestamp(Instant.now()).build();
        eventProducer.publishLeaderboardRefresh(contestId, ev);
        return ResponseEntity.ok().build();
    }
}